 ...
 *  Generated at:  Thu Jul 16 17:26:53 2015
 *
 ...
 * 04/25/14    2       OT       Anomaly 6642 - implemented EngOn defeat
 * 06/10/14    3       OT       Updates for QAC errors
 * 10/20/14    4       OT       Updates for STTd functionality                                                   12539
 * 07/16/15    5       OT       Updates for SCIR 003B                                                            13475
 * 08/14/15    6       OT       Updates for SCIR 003C                                                         EA3#2497
 * 09/22/15    7       OT       Corrected permanent/temporary control C-Term logic                            EA3#2931
 */
/**********************************************************************************************************************
...
#define D_RAMPIDLE_CNT_U08				0u
#define D_RAMPDOWN_CNT_U08				1u
#define D_RAMPUP_CNT_U08				2u
 ...
 *   Implicit S/R API:
 *   -----------------
 *   UInt8 Rte_IRead_VehPwrMd_Per1_DmdDelestDA_Cnt_u08(void)
 ...
 *********************************************************************************************************************/
	...
	VAR(boolean, AUTOMATIC) DMDDELESTDAValid_Cnt_T_lgc;
	...
	DmdDelestDA_Cnt_T_u08 = Rte_IRead_VehPwrMd_Per1_DmdDelestDA_Cnt_u08();
	...
	/* Ramp Logic */
	if( (FALSE == ATerm_Cnt_M_lgc) ||
		(FALSE == CTerm_Cnt_T_lgc) )
	{
	...
	{
		if( (InvalidMsgQ_Cnt_T_lgc == FALSE) &&
			(MissingMsgQ_Cnt_T_lgc == FALSE) &&
			(DataOtherFltMsgQ_Cnt_T_lgc == FALSE) )
		{
			DMDDELESTDAValid_Cnt_T_lgc = TRUE;
		}
		else
		{
			DMDDELESTDAValid_Cnt_T_lgc = FALSE;
		}
		
		if( RampStatus_Cnt_M_u08 == D_RAMPIDLE_CNT_U08 )
		{
			if( (DMDDELESTDAValid_Cnt_T_lgc == TRUE) &&
				(DmdDelestDA_Cnt_T_u08 == (uint8)kDMD_DELEST_DA_Power_Cut_Request) &&
				(VehSpdValid_Cnt_T_lgc == TRUE) &&
				(VehSpd_Kph_T_f32 < k_STTdEntryThresh_Kph_f32) &&
				(SystemState_Cnt_T_enum == RTE_MODE_StaMd_Mode_OPERATE) &&
				(OutputRampMult_Uls_T_f32 >= D_ONE_ULS_F32) )
			{
		...
		else if( RampStatus_Cnt_M_u08 == D_RAMPDOWN_CNT_U08 )
		{
			if( ( ( ( (DmdDelestDA_Cnt_T_u08 == (uint8)kDMD_DELEST_DA_Not_Requested) &&
					  (EngOn_Cnt_T_lgc == TRUE) ) ||
					( (DMDDELESTDAValid_Cnt_T_lgc == FALSE) &&
					  (EtatMT_Cnt_T_u08 == (uint8)kETAT_MT_Engine_Running) ) ) &&
				  (OutputRampMult_Uls_T_f32 <= D_STTDRAMPDOWNTARGET_ULS_F32) ) ||
				( (VehSpdValid_Cnt_T_lgc == TRUE) &&
				  (VehSpd_Kph_T_f32 > k_STTdExitThresh_Kph_f32) ) )
			{
		...
		else
		{
			if( OutputRampMult_Uls_T_f32 >= D_ONE_ULS_F32 )
			{
				OperRampRate_XpmS_T_f32 = k_RampUpRtLoSpd_UlspmS_f32;
				OperRampValue_Uls_T_f32 = D_ONE_ULS_F32;
				RampStatus_Cnt_M_u08 = D_RAMPIDLE_CNT_U08;
			}
			else
			{
				OperRampRate_XpmS_T_f32 = k_STTdRampUpRate_UlspmS_f32;
